...
const Quiz = ({ userData }) => {
    ...
    const [formData, setFormData] = useState({
        questions: Array(counter).fill({ question: '', options: Array(4).fill(''), correctOption: -1 })
    });
                ...
                <div class="row">
                    ...
                    <main role="main" class="col-md-8 col-lg-9 sidebar5">
                        ...
                        <div class="dashboard-header px-md-4" style={{ padding: "0px 0px 70px 0px" }}>
                            ...
                            <Description onSubjectChange={handleSubjectChange} 
                            ...
                            <Card className='addnewcard89'>
                                ...
                                <Card.Body className='addstutnet1 d-flex justify-content-between align-items-center '>
                                    ...
                                    <Form onSubmit={handleSubmit}>
                                        {formData.questions.map((question, questionIndex) => (
                                            <div key={questionIndex} className='question-container'>
                                                <h5 style={{ fontSize: '20px', fontWeight: '600' }}>Question {questionIndex + 1}</h5>
                                                <Form.Group className="mb-3" controlId={`question-${questionIndex}`}>
                                                    <Form.Control
                                                        type="text"
                                                        name={`question-${questionIndex}`}
                                                        value={question.question}
                                                        onChange={(e) => handleChange(questionIndex, 'question', e.target.value)}
                                                        required
                                                ...
                                                {[0, 1, 2, 3].map((optionIndex) => (
                                                    <div key={optionIndex}>
                                                        <Stack direction="horizontal" gap={3}>
                                                            <label className="-p" htmlFor={`option${questionIndex}-${optionIndex}`}>
                                                                Option {optionIndex + 1}:
                                                            </label>
                                                            <div className="p-">
                                                                <Form.Group className="mb-3" controlId={`option${questionIndex}-${optionIndex}`}>
                                                                    <Form.Control
                                                                        type="text"
                                                                        name={`option-${questionIndex}-${optionIndex}`}
                                                                        value={question.options[optionIndex]}
                                                                        onChange={(e) => handleChange(questionIndex, `option-${optionIndex}`, e.target.value)}
                                                                        required
                                                            ...
                                                            <div className="p-">
                                                                <input
                                                                    ...
                                                                    onChange={() => handleChange(questionIndex, `checkbox-${optionIndex}`, optionIndex)}
                                                                />
                                                            </div>
                                                        </Stack>
                                                    </div>
                                        ...
                                        <div className="button-container mt-5 d-flex gap-3">
                                            <Button onClick={()=>setCounter((pre)=>pre+1)} color="success" className="grnext py-2 px-5">
                                                + Add Question
                                            </Button>
                                            <Button type="submit" color="success" className="grnext py-2 px-5">
                                                Save
                                            </Button>
                                            <Button onClick={()=>setCounter((pre)=>pre-1)}  type="submit" color="success" className="grnext py-2 px-5">
                                                Cancel
                                            </Button>
                                        </div>
                                    </Form>
                                    ...
                                    <div>
                                        <img src="./img/Quiz-1.png" className="Quiz4" alt="Quiz" />
                                    </div>
                                </Card.Body>
                            </Card>
                        </div>
