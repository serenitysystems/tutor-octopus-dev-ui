...
import {
  getAttendenceRecordRouter,
  getStudentRecordBatchRouter,
  updateStudentAttendenceRouter,
} from "../../apicalls/Student";
...
const Attendencelist = () => {
  ...
  const [batch, setBatch] = useState(newset);
  ...
  const handleResponse = async (event) => {
    ...
    if (!selectedBatch || calenderDate === "") {
      toast.info(
        "Select the batch first and date for which you are taking attendence"
      );
    } else {
    ...
    <div>
      ...
      <div className="showrecord-1">
        <button
          ...
          className="btn btn-primary ms-2 showbutton"
        >
          Show Record
        </button>
                  ...
                  <td>
                    <button
                      ...
                      style={{
                        ...
                        background: statuses[student.id]
                          ? "#23954a2b"
                          : "#E9D7D7",
                      }}
                      className="av"
      ...
      <Stack direction="horizontal" gap={3} className="row56">
        ...
        <div className="p-2">
          ...
          <ul className="pagination">
            {Array.from(
              { length: Math.ceil(data.length / studentsPerPage) },
              (_, index) => (
                <li
                  key={index}
                  className={`page-item ${
      ...
      <Modal show={RecordModal} onHide={() => setRecordModal(false)}>
        ...
        <Modal.Body>
          <select
            className="batch89"
            id="batch"
            value={selectedRecordBatch}
            onChange={handleRecordBatchChange}
          >
            <option value="">Select Batch</option>
            <BatchList />
